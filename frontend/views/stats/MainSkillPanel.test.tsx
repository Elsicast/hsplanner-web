...
describe('<MainSkillSection> attack skills', () => {
  it('renders the attack damage hero instead of the weapon fallback', () => {
    ...
    expect(screen.getAllByText('命中伤害').length).toBeGreaterThan(0)
    expect(screen.getByText(/次攻击\/秒/)).toBeInTheDocument()
    expect(screen.getByText('武器伤害')).toBeInTheDocument()
    expect(screen.getByText('粉碎打击')).toBeInTheDocument()
    expect(screen.queryByText(/无伤害/)).not.toBeInTheDocument()
    expect(screen.getAllByText(/攻击伤害/).length).toBeGreaterThan(0)
  })

  it('keeps the empty state when no breakdown exists at all', () => {
    renderSection(null)
    expect(screen.getByText(/在技能页签选择主技能/)).toBeInTheDocument()
  })
})
